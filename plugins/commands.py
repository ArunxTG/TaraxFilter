...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    InlineKeyboardButton('⤬ ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘs ⤬', url=f'http://telegram.me/{temp.U_NAME}?startgroup=true')
                ],[
    ...
    if len(message.command) != 2:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_sticker("CAACAgUAAxkBAAIcNGUIl51y2Zw5tF-43nQlPRPSB_arAALWBQAC8YqBVzcGkRtsIfg5HgQ") 
    ...
    if data.startswith("sendfiles"):
        ...
        k = await client.send_message(chat_id=user,text=f"<b>📕FileName ➠ : <code>{files.file_name}</code> \n\n🔗FileSize ➠ : {get_size(files.file_size)}\n\n#Note: Idu Telegram File dhaan bindaas Ula Poii edukanum ‼️ Epdi Edukuradhu nu therilaya? Kila How to download irukum Paruga</b>", reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton('📂 Dᴏᴡɴʟᴏᴀᴅ Nᴏᴡ 📂', url=g)
                    ], [
        ...
        if settings['is_shortlink'] and user not in PREMIUM_USER:
            ...
            files = files_[0]
            g = await get_shortlink(chat_id, f"https://telegram.me/{temp.U_NAME}?start=file_{file_id}")
            k = await client.send_message(chat_id=message.from_user.id,text=f"<b>📕FileName ➠ : <code>{files.file_name}</code> \n\n🔗FileSize ➠ : {get_size(files.file_size)}\n\n#Note: Idu Telegram File dhaan bindaas Ula Poii edukanum ‼️ Epdi Edukuradhu nu therilaya? Kila How to download irukum Paruga</b>", reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton('📂 Dᴏᴡɴʟᴏᴀᴅ Nᴏᴡ 📂', url=g)
                        ], [
    ...
    if not files_:
        ...
        try:
            ...
            title = '@TG_showtime ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@TG_showtime ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@TG_showtime  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
        ]]
...
@Client.on_message(filters.command("xshortlink_info"))
...
@Client.on_message(filters.command("xset_tutorial"))
...
@Client.on_message(filters.command("xremove_tutorial"))
